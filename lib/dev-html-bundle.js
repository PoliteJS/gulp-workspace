...
module.exports = function(content) {
	var self = this;
	var dest = Workspace.getConfig().dest.dev;
    ...
    var coreFolder = config.source.core;
    ...
    if (scripts.length) {
        ...
        var bundlePath = path.normalize(path.join(process.cwd(), dest, coreFolder, bundleName));
        var bundleMapName = bundleName.replace('.js', '.js.map');
        var bundleMapPath = path.normalize(path.join(process.cwd(), dest, coreFolder, bundleMapName));
        ...
        $('script').remove();
        $('body').append('<script type="text/javascript" src="' + rpath + coreFolder + '/' + bundleName + '"></script>');
    }
    ...
    ['css','js'].forEach(function(ext) {
        ...
        var fpath = path.normalize(path.join(process.cwd(), dest, coreFolder, fname));

        if (!fs.existsSync(fpath)) {
            var fname = 'index.' + ext;
            fpath = path.normalize(path.join(process.cwd(), dest, coreFolder, fname));
        }

        if (fs.existsSync(fpath)) {
            switch (ext) {
                case 'css':
                    $('head').append('<link rel="stylesheet" href="' + rpath + coreFolder + '/' + fname + '">');
                    break;
                case 'js':
                    $('body').append('<script type="text/javascript" src="' + rpath + coreFolder + '/' + fname + '"></script>');
                    break;
            }
        }    
    });
...
function bundleHashName(files) {
    return files.reduce(function(previousValue, currentValue) {
        return path.basename(previousValue) + '--' + path.basename(currentValue);
    }).replace(/.js/g, '');
}
