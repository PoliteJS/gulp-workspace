...
var markdownTagLoader = require('./markdown-tag-loader');
...
module.exports = function(content) {
    ...
    var targetConfig = Workspace.getConfig().target.dev;
    ...
    // parse markdown-tags
    content = markdownTagLoader(content);
	...
	/**
     * Libs Management (javascript)
     */
    if (true === targetConfig.libs.bundle) {
        ...
        if (scripts.length) {
            ...
            if (!fs.existsSync(bundlePath)) {
                var bundleContent = uglify.minify(scripts, {
                    compress: targetConfig.libs.minify, // not working...
